...
**Data:** Landsat 8 scene `LC08_L1TP_141045_20260412`, downloadable from USGS EarthExplorer (earthexplorer.usgs.gov) or the USGS M2M API. Put the band GeoTIFFs and the `*_MTL.xml` into `/content/Data`, then run the pipeline cells in order. Without 6S installed it still runs, just on the fallback atmosphere.
